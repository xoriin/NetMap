...
def refresh(
    ...
    token_payload = validate_refresh_claims(payload.refresh_token)
    user = db.get(User, int(token_payload["sub"]))
    if user is None or not user.is_active:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid refresh token")

    prior_state = validate_refresh_token_state(db, user=user, refresh_token=payload.refresh_token)
...
def logout(
    ...
    token_state = validate_refresh_token_state(db, user=current_user, refresh_token=payload.refresh_token)
...
def forgot_password(
    ...
    # Always return 204 regardless of outcome to prevent user enumeration.
    user = db.scalar(
        select(User).where(
            or_(
    ...
    # Use the request Origin header so the link points to the frontend,
    # not the backend API, when they're on different origins/ports.
