...
- **Device times convert from UTC to browser-local time** — topology device responses now restore the UTC offsets SQLite drops for last-checked, created, and updated timestamps, so Overview, Inventory, and Device details no longer display them shifted by the browser's timezone.
- **Deleting a device clears every user's favourite** — device removal now explicitly deletes its per-user favourite rows as well as relationships, preventing deleted device ids from lingering in Overview favourites when SQLite foreign-key cascades are disabled.
